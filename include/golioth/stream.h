...
/// each block is uploaded to provide access to status and CoAP response codes.
///
/// Note that the \p callback_arg will be the same for all blocks.
///
/// @param path The path in stream to set (e.g. "my_obj")
/// @param content_type The content type of the object (e.g. JSON or CBOR)
/// @param cb A callback that will be called after each block is sent (can be NULL)
/// @param callback_arg An optional user provided argument that will be passed to \p cb (can be
///        NULL)
...
/// Free the context memory by calling \ref golioth_stream_blockwise_stop.
///
/// @param client The client handle from @ref golioth_client_create
